...
export function showDepositSuccessToast(amount: number, newBalance: number): void {
  showSuccessToast(
    `Successfully deposited ${amount.toFixed(4)} FLOW. New balance: ${newBalance.toFixed(4)} FLOW`
  );
}
...
export function showBetSuccessToast(betAmount: number, remainingBalance: number): void {
  showSuccessToast(
    `Bet placed: ${betAmount.toFixed(4)} FLOW. Remaining balance: ${remainingBalance.toFixed(4)} FLOW`
  );
}
...
export function showWithdrawalSuccessToast(amount: number, newBalance: number): void {
  showSuccessToast(
    `Successfully withdrew ${amount.toFixed(4)} FLOW. New balance: ${newBalance.toFixed(4)} FLOW`
  );
}
...
export function showInsufficientBalanceToast(
  ...
  const message = context === 'bet'
    ? `Insufficient house balance. You need ${shortfall.toFixed(4)} more FLOW to place this bet.`
    : `Insufficient house balance. You have ${currentBalance.toFixed(4)} FLOW but tried to withdraw ${requiredAmount.toFixed(4)} FLOW.`;
